...
class ExpectedTaskDQN(AbstractDQNAgent):
    """
    # Arguments
    ...
    def __init__(self, model, policy=None, test_policy=None, enable_double_dqn=False, enable_dueling_network=False,
                 dueling_type='avg', expectation_update_krate=0.9999, expected_computation=0.55, expected_deadline=1.25, *args, **kwargs):
        ...
        #self.expected_inputsize = 2
        self.expectation_update_krate = expectation_update_krate
        self.expected_computation = expected_computation
        self.expected_deadline = expected_deadline
        
        # self.expected_inputsize = 0
        # self.expected_computation = 0
        # self.expected_deadline = 0
    ...
    def forward(self, observation):
        ...
        #print('a', observation[-3:])
        # self.expected_inputsize = 0.9999 * self.expected_inputsize + 0.0001 * observation[-2]
        self.expected_computation = self.expectation_update_krate * self.expected_computation + (1 - self.expectation_update_krate) * observation[-3]
        self.expected_deadline = self.expectation_update_krate * self.expected_deadline + (1 - self.expectation_update_krate) * observation[-1]
    ...
    def backward(self, reward, terminal):
        ...
        self.memory.append(self.recent_observation, self.recent_action, reward, False,
        ...
        if self.step > self.nb_steps_warmup and self.step % self.train_interval == 0:
            ...
            terminal1_batch = []
            state1_batch = []
            #expected_task_quality = [self.expected_computation, self.expected_inputsize, self.expected_deadline]
            #print(self.expected_computation, self.expected_inputsize, self.expected_deadline)
            for e in experiences:
                #print(type(e.state0), type(e.state1), type(e.reward), type(e.action))
                # try:
                #     # fast way to concatenate list
                #     state0 = e.state0[:-3]
                #     state0[0:0] = expected_task_quality
                #     state1 = e.state1[:-3]
                #     state1[0:0] = expected_task_quality
                ...
                try:
                    # fast way to concatenate list
                    state1 = np.copy(e.state1)
                    #print(state1)
                    # print(state1[0][-3], self.expected_computation)
                    # print(state1[0][-1], self.expected_deadline)
                    state1[0][-3] = self.expected_computation
                    #state1[0][-2] = self.expected_inputsize
                    state1[0][-1] = self.expected_deadline
                    
                    state0_batch.append(e.state0)
                    # if (self.step > self.nb_actions / 2):
                    #     state1_batch.append(state1)
                    # else:
